...
alias setupeverything="touch ~/.hushlogin ; rm -rf /tmp/dotfiles ; rm -rf /tmp/paru-bin ; cd /tmp/ && git clone https://aur.archlinux.org/paru-bin.git && cd paru-bin && gpg --recv-keys E41FF9C889B141CC ; env CFLAGS='-march=native -O2 -pipe -fno-plt -fexceptions -Wp,-D_FORTIFY_SOURCE=2 -Wformat -Werror=format-security -fstack-clash-protection -fcf-protection' RUSTFLAGS='-C opt-level=2 -C target-cpu=native' MAKEFLAGS='-j$(nproc)' makepkg -si && cd /tmp/ && git clone https://gitlab.com/that1communist/dotfiles.git && cd ./dotfiles && /bin/sudo chown -R $USER:$USER ./.* && chmod -R 750 ./.* && cp -r ./.* ~/ ; /bin/sudo cp -r ./etc / ; cd ~ ; /bin/sudo sed -i 's/#ParallelDownloads = 5/ParallelDownloads = 20/g' /etc/pacman.conf && /bin/sudo usermod -a -G video $LOGNAME && env CFLAGS='-march=native -O2 -pipe -fno-plt -fexceptions -Wp,-D_FORTIFY_SOURCE=2 -Wformat -Werror=format-security -fstack-clash-protection -fcf-protection' RUSTFLAGS='-C opt-level=2 -C target-cpu=native' MAKEFLAGS='-j$(nproc)' paru --sudo=sudo --needed -Syu && env CFLAGS='-march=native -O2 -pipe -fno-plt -fexceptions -Wp,-D_FORTIFY_SOURCE=2 -Wformat -Werror=format-security -fstack-clash-protection -fcf-protection' RUSTFLAGS='-C opt-level=2 -C target-cpu=native' MAKEFLAGS='-j$(nproc)' paru --sudo=sudo --needed -S alacritty alsa-tools alsa-utils ananicy-cpp-git cachyos-ananicy-rules-git  android-udev ark bat bato bluez-utils bzip2 ccache chezmoi copyq cpupower doasedit dua-cli earlyoom easyeffects eza ffmpegthumbnailer ffmpegthumbs firefox flat-remix galculator gamemode gamescope gammastep glfw-wayland gnome-keyring gparted grim gst-plugin-pipewire gtk3-classic gucharmap gvfs gvfs-mtp gzip handlr-regex loupe iwgtk jaq kanshi keyd-git kvantum kvantum-qt5 tesseract tesseract-data-eng kwayland-integration lib32-libva-mesa-driver lib32-pipewire-jack lib32-vkd3d lib32-vulkan-icd-loader libappindicator-gtk3 libva-mesa-driver light linux-firmware mako mpv neovim neovim-coc-bin neovim-symlinks noto-fonts noto-fonts-cjk noto-fonts-emoji noto-fonts-extra opendoas otf-font-awesome p7zip pavucontrol pbzip2 pcmanfm-qt pigz pinta polkit-gnome profile-sync-daemon proton-ge-custom-bin python-rarfile python-enzyme deluge-gtk protonhax-git protontricks qt5-wayland qt5ct qt6-wayland qt6ct qtalarm realtime-privileges reflector resources ripgrep rsync seahorse slurp speedread-git steam subliminal swaddle swaybg swayidle swaylock-effects-git hyprland hyprland-autoname-workspaces-git sd thermald thunderbird tofi ttf-nerd-fonts-symbols-mono unarchiver unrar vkd3d vim-hexokinase vulkan-headers vulkan-mesa-layers vulkan-validation-layers waybar wine-gecko wine-mono wine-staging winetricks wljoywake wl-clipboard wl-mirror xdg-desktop-portal-hyprland xdg-user-dirs xorg-xwayland xz zim zsh-autosuggestions zsh-syntax-highlighting ; touch -a ~/.config/hypr/$HOSTNAME.conf ; handlr set application/x-extension-htm firefox.desktop ; handlr set application/x-extension-html firefox.desktop ; handlr set application/x-extension-shtml firefox.desktop ; handlr set application/x-extension-xht firefox.desktop ; handlr set application/x-extension-xhtml firefox.desktop ; handlr set application/xhtml+xml firefox.desktop ; handlr set 'image/*' org.gnome.Loupe.desktop ; handlr set image/png org.gnome.Loupe.desktop ; handlr set image/jpeg org.gnome.Loupe.desktop ; handlr set image/gif mpv.desktop ; handlr set 'video/*' mpv.desktop ; handlr set x-scheme-handler/about firefox.desktop ; handlr set x-scheme-handler/chrome firefox.desktop ; handlr set x-scheme-handler/feed firefox.desktop ; handlr set x-scheme-handler/file pcmanfm-qt.desktop ; handlr set x-scheme-handler/ftp firefox.desktop ; handlr set x-scheme-handler/http firefox.desktop ; handlr set x-scheme-handler/https firefox.desktop ; handlr set inode/directory pcmanfm-qt.desktop ; handlr set x-scheme-handler/steam steam.desktop ; /bin/sudo systemctl enable thermald ; /bin/sudo systemctl enable earlyoom ; /bin/sudo systemctl enable lm_sensors ; /bin/sudo systemctl enable ananicy-cpp ; /bin/sudo systemctl enable paccache ; /bin/sudo systemctl enable systemd-timesyncd.service ; systemctl --user enable gamemoded ; systemctl --user enable psd.service ; /bin/sudo systemctl enable keyd ; systemctl --user enable wayland-pipewire-idle-inhibit.service ; xdg-user-dirs-update ; mkdir -p ~/Pictures/.clipboard/ ; mkdir -p ~/.local/share/zsh/ ; mkdir ~/Torrents ; /bin/sudo sd -f i 'timeout 3' 'timeout 0' /boot/loader/loader.conf ; /bin/sudo sd -f i 'options root' 'options quiet loglevel=3 rd.systemd.show_status=auto rd.udev.log_priority=3 nowatchdog root' /boot/loader/entries/*.conf ; /bin/sudo sd -f i 'username' $USER /etc/systemd/system/getty@tty1.service.d/skip-prompt.conf ; sd -f i 'username' $USER ~/.config/mpv/scripts/autosub.lua ; /bin/sudo sd -f i 'username' $USER ~/.config/pcmanfm-qt/default/settings.conf ; /bin/sudo sd -f i 'username' $USER ~/.config/gtk-3.0/bookmarks ; /bin/sudo sd -f i 'relatime' 'noatime' /etc/fstab ; gsettings set com.github.wwmm.easyeffects.streaminputs plugins \"['rnnoise#0', 'speex#0', 'echo_canceller', 'compressor']\" ; gsettings set com.github.wwmm.easyeffects.speex:/com/github/wwmm/easyeffects/streaminputs/speex/0/ enable-denoise false ; gsettings set com.github.wwmm.easyeffects.speex:/com/github/wwmm/easyeffects/streaminputs/speex/0/ enable-dereverb true ; chmod +x ~/.config/tofi/scripts/power.sh ; chsh -s /bin/zsh ; /bin/sudo chsh -s /bin/zsh ; touch ~/.config/hypr/$HOSTNAME.conf ; pw-play --volume=0.2 '/usr/share/sounds/freedesktop/stereo/complete.oga'"
alias updatedots="cd /tmp/ ; rm -rf ./dotfiles ; git clone https://gitlab.com/that1communist/dotfiles.git && cd dotfiles && doas chown -R $USER:$USER ./.* && doas chmod -R 750 ./.* && rm -rf ./.config/fontconfig ; rm -rf ./.config/gtk-3.0 ; rm -rf ./.config/handlr ; rm -rf ./.config/kanshi ; rm -rf ./.config/qBittorrent ; rm -rf ./.config/qt5ct rm -rf ./.config/mimeapps.list ; rm -rf ./.config/QTalarm/ ; rm -rf ./.config/syncplay ; rm -rf ./.config/galculator ; rm -rf ./.config/mpv/ ; rm -rf ./.config/gammastep ; rm -rf ./.config/firefox ; rm -rf .config/sway/wallpaper && cp -r ./.* ~/ && doas cp -r ./etc / ; cd ~ ; doas sd -f i 'username' $USER /etc/systemd/system/getty@tty1.service.d/skip-prompt.conf ; doas sd -f i 'username' $USER ~/.config/qBittorrent/qBittorrent.conf ; doas sd -f i 'username' $USER ~/.config/pcmanfm-qt/default/settings.conf ; doas sd -f i 'username' $USER ~/.config/gtk-3.0/bookmarks ; doas sd -f i 'relatime' 'noatime' /etc/fstab ; zsh ; pw-play --volume=0.2 '/usr/share/sounds/freedesktop/stereo/complete.oga'" 
