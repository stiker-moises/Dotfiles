...
alias setupeverything="touch ~/.hushlogin ; rm -rf /tmp/dotfiles ; rm -rf /tmp/paru-bin ; cd /tmp/ && git clone https://aur.archlinux.org/paru-bin.git && cd paru-bin && gpg --recv-keys E41FF9C889B141CC ; env CFLAGS='-march=native -O2 -pipe -fno-plt -fexceptions -Wp,-D_FORTIFY_SOURCE=2 -Wformat -Werror=format-security -fstack-clash-protection -fcf-protection' RUSTFLAGS='-C opt-level=2 -C target-cpu=native' MAKEFLAGS='-j$(nproc)' makepkg -si && cd /tmp/ && git clone https://gitlab.com/that1communist/dotfiles.git && cd ./dotfiles && /bin/sudo chown -R $USER:$USER ./.* && chmod -R 750 ./.* && /bin/sudo cp -r ./etc / && cp -r ./.* ~/ ; cd ~ ; /bin/sudo sed -i 's/#ParallelDownloads = 5/ParallelDownloads = 20/g' /etc/pacman.conf && /bin/sudo usermod -a -G video $LOGNAME && paru --sudo=sudo -S --noconfirm reflector && /bin/sudo reflector --protocol https --latest 20 --fastest 5 --sort rate --save /etc/pacman.d/mirrorlist && env cflags='-march=native -o2 -pipe -fno-plt -fexceptions -wp,-d_fortify_source=2 -wformat -werror=format-security -fstack-clash-protection -fcf-protection' rustflags='-c opt-level=2 -c target-cpu=native' makeflags='-j$(nproc)' paru --sudo=sudo --needed -syu ; env cflags='-march=native -o2 -pipe -fno-plt -fexceptions -wp,-d_fortify_source=2 -wformat -werror=format-security -fstack-clash-protection -fcf-protection' rustflags='-c opt-level=2 -c target-cpu=native' makeflags='-j$(nproc)' paru --sudo=sudo --needed -s alacritty alsa-tools alsa-utils ananicy-cpp-git blueberry cachyos-ananicy-rules-git  android-udev ark bat bato bluez-utils bzip2 ccache copyq cpupower doasedit dua-cli earlyoom easyeffects eza ffmpegthumbnailer ffmpegthumbs firefox flat-remix galculator gamescope gammastep glfw-wayland gnome-keyring gparted grim gst-plugin-pipewire gtk3-classic gvfs gvfs-mtp gzip handlr-regex loupe iwgtk jaq kanshi keyd-git kvantum kvantum-qt5 tesseract tesseract-data-eng kwayland-integration lib32-libva-mesa-driver lib32-pipewire-jack lib32-vkd3d lib32-vulkan-icd-loader libappindicator-gtk3 libva-mesa-driver light linux-firmware mako mpv neovim neovim-coc-bin neovim-symlinks noto-fonts noto-fonts-cjk noto-fonts-emoji noto-fonts-extra opendoas otf-font-awesome p7zip pavucontrol pbzip2 pcmanfm-qt pigz pinta polkit-gnome power-profiles-daemon profile-sync-daemon proton-ge-custom-bin deluge-gtk protonhax-git protontricks qt5-wayland qt5ct qt6-wayland qt6ct qtalarm realtime-privileges reflector resources ripgrep rsync slurp speedread-git steam subliminal swaybg swayidle swaylock-effects-git hyprland hyprkeys hyprland-autoname-workspaces-git sd thunderbird tofi ttf-nerd-fonts-symbols-mono unarchiver unrar vkd3d vim-hexokinase vulkan-headers vulkan-mesa-layers vulkan-validation-layers waybar wayland-pipewire-idle-inhibit wine-gecko wine-mono wine-staging winetricks wljoywake wl-clipboard wl-mirror xdg-desktop-portal-hyprland xdg-user-dirs xorg-xwayland xz zim zsh-autosuggestions zsh-syntax-highlighting ; touch -a ~/.config/hypr/$(cat /etc/hostname).conf ; handlr set application/x-extension-htm firefox.desktop ; handlr set application/x-extension-html firefox.desktop ; handlr set application/x-extension-shtml firefox.desktop ; handlr set application/x-extension-xht firefox.desktop ; handlr set application/x-extension-xhtml firefox.desktop ; handlr set application/xhtml+xml firefox.desktop ; handlr set 'image/*' org.gnome.loupe.desktop ; handlr set image/png org.gnome.loupe.desktop ; handlr set image/jpeg org.gnome.loupe.desktop ; handlr set image/gif mpv.desktop ; handlr set 'video/*' mpv.desktop ; handlr set x-scheme-handler/about firefox.desktop ; handlr set x-scheme-handler/chrome firefox.desktop ; handlr set x-scheme-handler/feed firefox.desktop ; handlr set x-scheme-handler/file pcmanfm-qt.desktop ; handlr set x-scheme-handler/ftp firefox.desktop ; handlr set x-scheme-handler/http firefox.desktop ; handlr set x-scheme-handler/https firefox.desktop ; handlr set inode/directory pcmanfm-qt.desktop ; handlr set x-scheme-handler/steam steam.desktop ; /bin/sudo systemctl enable bluetooth.service ; /bin/sudo systemctl enable earlyoom ; /bin/sudo systemctl enable lm_sensors ; /bin/sudo systemctl enable ananicy-cpp ; /bin/sudo systemctl enable power-profiles-daemon ; /bin/sudo systemctl enable paccache ; /bin/sudo systemctl enable systemd-timesyncd.service ; systemctl --user enable gamemoded ; systemctl --user enable psd.service ; /bin/sudo systemctl enable keyd ; xdg-user-dirs-update ; mkdir -p ~/pictures/.clipboard/ ; mkdir -p ~/.local/share/zsh/ ; mkdir ~/torrents ; /bin/sudo sd -f i 'timeout 3' 'timeout 0' /boot/loader/loader.conf ; /bin/sudo sd -f i 'options root' 'options quiet loglevel=3 rd.systemd.show_status=auto rd.udev.log_priority=3 nowatchdog root' /boot/loader/entries/*.conf ; /bin/sudo sd -f i 'username' $user /etc/systemd/system/getty@tty1.service.d/skip-prompt.conf ; sd -f i 'username' $user ~/.config/mpv/scripts/autosub.lua ; sd -f i 'username' $user ~/.config/pcmanfm-qt/default/settings.conf ; sd -f i 'username' $user ~/.config/gtk-3.0/bookmarks ; /bin/sudo sd -f i 'relatime' 'noatime' /etc/fstab ; gsettings set com.github.wwmm.easyeffects.streaminputs plugins \"['rnnoise#0', 'speex#0', 'echo_canceller', 'compressor']\" ; gsettings set com.github.wwmm.easyeffects.speex:/com/github/wwmm/easyeffects/streaminputs/speex/0/ enable-denoise false ; gsettings set com.github.wwmm.easyeffects.speex:/com/github/wwmm/easyeffects/streaminputs/speex/0/ enable-dereverb true ; chmod +x ~/.config/tofi/scripts/power.sh ; chsh -s /bin/zsh ; /bin/sudo chsh -s /bin/zsh ; touch ~/.config/hypr/$hostname.conf ; /bin/sudo ln -s /usr/bin/jaq /usr/bin/jq ; pw-play --volume=0.2 '/usr/share/sounds/freedesktop/stereo/complete.oga'"
